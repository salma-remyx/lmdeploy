...
- \[2026/08\] Add an opt-in attention-memory probe. `LMDEPLOY_ATTN_MEM_PROBE=1` checks paged-KV, compressed-KV, sparse-index and recurrent-state reads at the attention boundary and accumulates a tiered risk ledger that localizes failures to eviction / slot-reuse regimes.
